...
def test_a_permanent_failure_is_marked_failed_and_acked(reason):
...
def test_any_other_failure_is_left_to_redeliver(reason):
    did_store, queue = FakeDidStore(), FakeQueue()
...
def test_any_other_failure_on_the_final_delivery_is_dead_lettered_without_an_ack(reason):
    did_store, queue = FakeDidStore(), FakeQueue()
...
def test_a_flush_failure_dead_letters_only_final_deliveries():
