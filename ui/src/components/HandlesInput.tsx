  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-medium text-zinc-700">
        User Handles <span className="font-normal text-zinc-400">(optional)</span>
      </label>
      ...
      {value.length > 0 && (
        <ul className="flex flex-wrap gap-2">
          {value.map((handle) => (
            <li key={handle} className="flex items-center gap-1 rounded-full bg-zinc-100 px-3 py-1 text-sm text-zinc-700">
              {handle}
              <button
                type="button"
