...
Concatenate the per-candidate Parquet files into `posts.parquet` and write `metadata.json`. No Athena. No S3.

## Scope

- **Caller:** `experiments/democratic_candidate_posts_2026_09_09/main.py` → `run()` (Step 5). Immediate caller is `compile_outputs(...)`.
- **Task:** Concatenate rows, preserve per-candidate duplicates, write combined Parquet and metadata.
- **Out of scope:** Athena, S3, CLI, product packages.

## Files

### Inspect

- `experiments/democratic_candidate_posts_2026_09_09/schemas.py` (`COMBINED_COLUMNS`, metadata keys from Step 1)
- `experiments/democratic_candidate_posts_2026_09_09/constants.py` (candidate table)
...
- `experiments/democratic_candidate_posts_2026_09_09/compile.py` (create)
- `tests/experiments/democratic_candidate_posts_2026_09_09/test_compile.py` (create)

### Forbidden to change

- `experiments/democratic_candidate_posts_2026_09_09/sql.py`
...
Caller = `compile_outputs`. Unit of work: local files only.
...
In `tests/experiments/democratic_candidate_posts_2026_09_09/test_compile.py`:
